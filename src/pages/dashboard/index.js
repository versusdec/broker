import {
  ...
  Table,
  Typography,
  Unstable_Grid2 as Grid, SvgIcon, Tooltip, IconButton, Alert
} from '@mui/material';
...
import {Close, DeleteOutlined, EditOutlined, FileOpenOutlined, HelpOutline, QuestionAnswerOutlined, UnarchiveOutlined} from "@mui/icons-material";
...
const Page = () => {
  ...
  const projectsData = useProjects(useMemo(() => ({
    status: 'active',
    limit: '5'
  }), []))
  
  const ticketsData = useTickets(useMemo(() => ({
    status: 'active',
    limit: '5'
  }), []))
  ...
  useEffect(() => {
    if (ticketsData.data?.items) setTickets(ticketsData.data.items)
  }, [ticketsData])
          ...
          <Grid xs={12} md={8}>
            <Stack spacing={3}>
              <Card>
                ...
                <Box mb={5}>
                  <Table>
                    {projectsData.loading && !Boolean(projects) &&
                    <TableBody>
                      <TableRow>
                        <TableCell width={'100%'}>
                          <Box sx={{
                    ...
                    {!projectsData.loading && !Boolean(projects?.length) &&
                    <TableBody>
                      <TableRow>
                ...
                <CardContent>
                  <Stack spacing={3}>
                    <Stack direction={'row'} alignItems={'center'} justifyContent={'space-between'} spacing={2}>
                      ...
                      <Button variant={'text'} component={NextLink} href={paths.support.index}>See All Appeals</Button>
                    </Stack>
                  </Stack>
                </CardContent>
                <Box mb={5}>
                  <Table>
                    {ticketsData.loading && !Boolean(tickets) &&
                    <TableBody>
                      <TableRow>
                        <TableCell width={'100%'}>
                          <Box sx={{
                    ...
                    {!ticketsData.loading && !Boolean(tickets?.length) &&
                    <TableBody>
                      <TableRow>
                        <TableCell align={'center'}>
                          <Typography variant={'subtitle2'}>
              ...
              <CardContent>
                <Stack spacing={3}>
                  ...
                  <Stack direction={'row'} spacing={2} alignItems={'center'}>
                    <Box>
                      <svg width="64" height="64" viewBox="0 0 72 74" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <g opacity="0.5" filter="url(#filter0_f_305_33074)">
                          <rect x="21.1191" y="42.1212" width="20.3636" height="19.7143" rx="5" transform="rotate(-90 21.1191 42.1212)" fill="#8237FF" fillOpacity="0.5"/>
                        </g>
